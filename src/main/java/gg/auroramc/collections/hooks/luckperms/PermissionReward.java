...
public class PermissionReward extends AbstractReward {
    ...
    public void execute(Player player, long level, List<Placeholder<?>> placeholders) {
        ...
        AuroraCollections.logger().debug("Adding permission " + node.getKey() + " to player " + player.getName());
    ...
    public void init(ConfigurationSection args) {
        ...
        if (permission == null) {
            AuroraCollections.logger().warning("PermissionReward doesn't have the permission key");
        }
    }
